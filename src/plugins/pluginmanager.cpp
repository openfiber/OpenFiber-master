    ...
    mPlugins(Plugins())
{
    ...
    // OpenFiber on Windows or Linux AND from within Qt Creator, then the binary
    // will be running from [OpenFiber]/build/OpenFiber[.exe] rather than
    // [OpenFiber]/build/bin/OpenFiber[.exe] as it should if we were to mimic the
    // case where OpenFiber has been deployed. Then, because the plugins are in
    // [OpenFiber]/build/plugins/OpenFiber, we must skip the "../" bit. Yes, it's
